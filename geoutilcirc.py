...
def create_partinioned_bounding_box(box_length, partition_radius):
	...
	partitions_row_count = int(box_length / partition_diameter) + 1
	partitions = []
	...
	for i in range(partitions_row_count):
		...
		row_start = distance_calculator.destination(bounding_box[0], 180, 
			i *	partition_diameter)
		for j in range(partitions_row_count):
			partition_start = distance_calculator.destination(row_start, 90, j * 
				partition_diameter)
			center = distance_calculator.destination(partition_start, 
				135, sqrt(2)*partition_diameter)
...
def find_partition_to_point(partitions, point):
	...
	for row_index in range(len(partitions)):
		...
		partition_row_start = get_coord_of_corner(
							partition_row[0][0], partition_row[0][1])
		partition_row_end = get_coord_of_corner(
							partition_row[-1][0], partition_row[-1][1], 135)
		...
		if is_in_area(partition_row_start, 
			...
			for column_index in range(len(partition_row)):
				...
				if is_in_circle(partition[0], partition[1], point):
